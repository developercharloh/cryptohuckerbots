                ...
                <div key={txn.id} className="flex items-center justify-between px-4 py-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1 text-[11px] text-muted-foreground">
                      ...
                      <span className="capitalize">{txn.type === 'trade_loss' ? 'Contract Opened' : txn.type === 'trade_profit' || txn.type === 'trade_loss_return' ? 'Contract Closed' : txn.type.replace('_', ' ')}</span>
                      <span>·</span>
                      <span>{txn.status}</span>
                    </div>
                  </div>
                  <div className={`text-sm font-semibold ml-3 ${txn.type === 'deposit' || txn.type === 'trade_profit' ? 'text-emerald-400' : txn.type === 'withdrawal' || txn.type === 'trade_loss' || txn.type === 'trade_loss_return' ? 'text-red-400' : 'text-foreground'}`}>
